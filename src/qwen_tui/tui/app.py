...
• Status panel shows system information"""
    ...
    async def send_message(self, message: str) -> None:
        ...
        try:
            ...
            await self.history_manager.save_message(
                user_message, backend_name, model_name
            )
        except Exception as e:
        ...
        try:
            ...
            if self.thinking_manager:
                response_content = ""
                async for chunk in self.thinking_manager.think_and_respond(
                ...
                request = LLMRequest(
                    messages=self.conversation_history.copy(), stream=True
                )
                full_response = ""
                ...
                response_content, thinking_content = self._filter_thinking_tags(
                    full_response
                )
                ...
                if self.current_thinking_widget and thinking_content:
                    ...
                    self.current_thinking_widget.update_thinking_text(
                        "Found internal reasoning in response"
                    )
                ...
                if self.current_thinking_widget:
                    self.current_thinking_widget.stop_thinking()
                    self.current_thinking_widget.update_thinking_text(
                        "Used direct backend with thinking filtering"
                    )
                    self.current_thinking_widget = None
            ...
            if response_content:
                ...
                try:
                    await self.history_manager.save_message(
                        assistant_message, backend_name, model_name
                    )
                except Exception as e:
                    sys_msg = ChatMessage(
                        "system", f"Failed to save assistant message: {e}"
                    )
                    await chat_scroll.mount(sys_msg)
                    sys_msg.scroll_visible()
    ...
    async def handle_command(self, command: str) -> None:
        ...
        if cmd == "help":
            ...
            self.add_system_message(
                f"Backend switching to {backend_name} (not implemented yet)"
            )
            ...
            if len(args) >= 2:
                ...
                session_id, format_type = self.current_session_id or "", (
                    args[0] if args else "json"
                )
            asyncio.create_task(
                self._export_conversation_session(session_id, format_type)
            )
    ...
    async def _show_conversation_history(self) -> None:
        ...
        try:
            ...
            if not sessions:
                no_history_message = ChatMessage(
                    "system", "No conversation history found."
                )
    ...
    async def _load_conversation_session(self, session_id: str) -> None:
        ...
        try:
            ...
            sys_msg = ChatMessage(
                "system", f"Loaded conversation session: {session_id}"
            )
            await chat_scroll.mount(sys_msg)
            sys_msg.scroll_visible()
    ...
    async def _export_conversation_session(
        self, session_id: str, format_type: str
    ) -> None:
        ...
        try:
            ...
            success = await self.history_manager.export_session(
                session_id, export_file, format_type
            )

            chat_scroll = self.query_one("#chat-scroll", ScrollableContainer)
            if success:
                success_msg = ChatMessage(
                    "system", f"Exported conversation to: {export_file}"
                )
    ...
    async def _handle_permission_command(self, args: list) -> None:
        ...
        if subcommand == "clear":
            if len(args) < 2:
                error_msg = ChatMessage(
                    "error", "Usage: /permissions clear <tool_name>"
                )
                await chat_scroll.mount(error_msg)
                error_msg.scroll_visible()
                return
            ...
            success_msg = ChatMessage(
                "system", f"Cleared preferences for tool: {tool_name}"
            )
            await chat_scroll.mount(success_msg)
            success_msg.scroll_visible()
            ...
            if self.permission_manager.yolo_mode:
                ...
                mode_msg = ChatMessage(
                    "system", "🛡️ YOLO mode disabled. Security checks re-enabled."
                )
            else:
                self.permission_manager.yolo_mode = True
                mode_msg = ChatMessage(
                    "system", "⚠️ YOLO mode enabled. All security checks bypassed!"
                )
...
class ActionWidget(Static):
    ...
    def update_display(self):
        ...
        if self.parameters:
            params_str = ", ".join(
                [f"{k}={v}" for k, v in list(self.parameters.items())[:2]]
            )
        ...
        if self.result:
            result_preview = (
                self.result[:100] + "..." if len(self.result) > 100 else self.result
            )
