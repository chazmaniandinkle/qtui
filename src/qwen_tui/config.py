...
class OllamaConfig(BaseModel):
    ...
    keep_alive: Union[int, str] = Field(
        default="5m", description="Model keep alive time"
    )
...
class VLLMConfig(BaseModel):
    ...
    model: str = Field(
        default="Qwen/Qwen2.5-Coder-7B-Instruct", description="Model path"
    )
    timeout: int = Field(default=300, description="Request timeout in seconds")
...
class SecurityConfig(BaseModel):
    ...
    profile: SecurityProfile = Field(
        default=SecurityProfile.BALANCED, description="Security profile"
    )
    allowed_commands: List[str] = Field(
        default_factory=list, description="Allowed shell commands"
    )
    blocked_commands: List[str] = Field(
        default_factory=list, description="Blocked shell commands"
    )
    allow_network: bool = Field(default=True, description="Allow network operations")
    allow_file_write: bool = Field(
        default=True, description="Allow file write operations"
    )
    allow_file_delete: bool = Field(
        default=False, description="Allow file delete operations"
    )
...
class UIConfig(BaseModel):
    ...
    animation_speed: float = Field(
        default=1.0, description="Animation speed multiplier"
    )
    show_typing_indicator: bool = Field(
        default=True, description="Show typing indicator"
    )
    auto_scroll: bool = Field(default=True, description="Auto-scroll chat")
    font_size: str = Field(
        default="normal", description="Font size: small, normal, large"
    )
...
class Config(BaseModel):
    ...
    openrouter: OpenRouterConfig = Field(
        default_factory=lambda: OpenRouterConfig(api_key="")
    )
    ...
    parallel_tools: int = Field(
        default=3, description="Maximum parallel tool executions"
    )
    cache_responses: bool = Field(default=True, description="Cache LLM responses")

    # Protocol server
    use_protocol: bool = Field(
        default=False, description="Use protocol server for backend operations"
    )
    protocol_url: str = Field(
        default="ws://localhost:8765/ws", description="URL of protocol server"
    )
